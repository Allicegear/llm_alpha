...
* **执行步骤**:
    ...
    2.  **中性化方式选择**: 根据Alpha策略特性选择具体的中性化方式：
        * **FAST**: Neutralization: `Fast Factors`. 适用于高换手率信号（>70%年化）、短期趋势捕捉、高频数据或短期价格动量策略
        * **SLOW**: Neutralization: `Slow Factors`.适用于低换手率信号（<50%年化）、长期趋势捕捉、基本面或价值策略
        * **SLOW_AND_FAST**: Neutralization: `Slow + Fast Factors`. 适用于同时考虑短期和长期趋势的策略、多元化Alpha组合、平衡型投资策略
        * **CROWDING**: Neutralization: `Slow + Fast Factors`.适用于希望降低拥挤交易风险的策略、强动量市场环境、流行因子或拥挤交易策略
        * **REVERSION_AND_MOMENTUM**: Neutralization: `RAM`.适用于需要对冲短期均值回归和长期动量的策略、平衡型多空Alpha因子
        * **STATISTICAL**: Neutralization: `STATISTICAL`.适用于需要捕捉复杂非线性关系的策略、基于大量变量构建的因子、数据驱动型策略
...
- **Region**: `GLB` # 国家或地区
- **Delay**:  `1` # 数据延迟
- **Universe**: `MINVOL1M` # 研究目标股票池（如TOP3000、TOP1000等）
- **Dataset**: `fundamental31` # 研究目标数据集
