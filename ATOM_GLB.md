...
### **1. The Rule of 4 (Batch Survival Law)**

- **Directive**: Any `create_multiSim` call **must** and **always** contain **4** different Alpha expressions.
...
- **Directive**: If `check_multisimulation_status` shows `in_progress` (or stuck in `pending` without moving to `simulating`) for more than **15 minutes**, it is considered stuck/zombie.
...
| --- | --- |
| **High Turnover** | `trade_when`, Decay 5-10, `ts_mean`, `ts_decay_linear`, `ts_target_tvr_decay`. |
| **Low Margin (< 5bps)** | **Goal**: Margin/Turnover > 1.2.   1. Increase `decay`.   2. Add `trade_when` filter.   3. Expand Window (5->22->66). |
...
1. **Strategy Selection**: Build 4 ATOM expressions based on Phase 1 intelligence.
...
1. **Submission**: `create_multiSim(visualization=false)`.
    - **validation**: before calling, verify the 4 expressions are valid.
    - **Pre-flight**: Re-verify `universe` and `delay` against `get_platform_setting_options` for `GLB` region before calling.
...
3. **[CRITICAL OUTPUT]**: After each backtest ends, **you must output the following**:
    ...
    - **Proposed Expressions**: The 4 codes for the next batch.
...
2. **Correlation Check**: **Only after** passing all above IS standards, call `check_correlation`.
    - If SelfCorr/ProdCorr > 0.7 -> Modify logic, return to Phase 4.
...
Please immediately initialize the environment (get_platform_setting_options), lock the Target Region / Target Dataset, and begin mining the first round of ATOM-Alphas.