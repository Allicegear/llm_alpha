...
#### **1. 批量生存法则 (The Rule of 4)**

* **指令**: 任何一次 `create_multiSim` 调用，**必须**且**始终**包含 **4 个** 不同的 Alpha 表达式。
* **原因**: 单次提交过少会触发 `"At least 2 alpha expressions required"` 错误，且浪费迭代机会。
* **应对**: 如果你只想测试 1 个逻辑，必须立即生成 3 个该逻辑的变体（改变窗口期、Decay值或算子），凑齐 4 个一并提交。
...
1.  **Step 1: 模板选择与填充 (Template Selection & Population)**
    ...
    * **操作步骤**:
        ...
        5.  **变体生成**: 基于选定的模板和填充的字段，生成 4 个不同变体，**仅通过调整模板中已定义的参数、算子组合或中性化方式**来增加多样性，**不得引入任何模板外的逻辑或结构**。
...
2.  **Step 2: 表达式验证与标准化 (Expression Validation & Standardization)**
    ...
    * **最终检查**: 检查生成的 4 个表达式是否足够多样化，是否覆盖了不同的策略视角和参数组合，同时确保所有表达式中的算子都在用户权限范围内。
...
1.  **正式提交**:
    ...
    * **执行提交**: 调用 `create_multiSim` (包含 4 个基于模板填充的表达式和正确的中性化设置)。
...
3.  **生成下一代**: 基于外部情报或内部诊断，**仅从 Templates/模板库.md 中选择新的模板或调整现有模板的填充方式**，生成 4 个新变体，**不得进行任何无模板基础的信号构建**。
4.  **重跑**: 返回 Phase 3。
...
- **Region**: `GLB` # 国家或地区
- **Delay**:  `1` # 数据延迟
- **Universe**: `TOPDIV3000` # 研究目标股票池（如TOP3000、TOP1000等）
- **Dataset**: `option4` # 研究目标数据集
